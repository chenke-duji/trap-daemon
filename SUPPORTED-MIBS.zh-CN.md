# Supported SNMP MIBs（已支持的 SNMP MIB 清单）

> 本清单按「厂商（Vendor）→ MIB 名称」两级列出 trap-daemon 已支持的 SNMP MIB
> （通过 `oid-database.db` 将 trap varbind OID 映射为字段名）。
>
> - 数据来源：mib-parser 生成的 `oid-database.db`（与 cep-engine 的
>   `*-MIB.parser.groovy` 脚本覆盖范围一致）。
> - Vendor 依据文件名前缀识别（无前缀的标准 MIB 归入 IETF-STD）。
> - 更新日期：2026-09-10
